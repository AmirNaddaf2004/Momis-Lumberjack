...
The tournament has now officially ended. Keep practicing for the next event!`;
    ...
    const options = {
        parse_mode: 'Markdown',
        reply_markup: {
            inline_keyboard: [[{ text: '🚀 Play in Free mode and practice!', web_app: { url: 'https://lumberjack.momis.studio' } }]]
        }
    };
    try {
        await bot.sendMessage(telegramId, message, options);
...
function startListening() {
    bot.onText(/\/start/, (msg) => {
        ...
        const options = {
            parse_mode: 'Markdown',
            reply_markup: {
                inline_keyboard: [[{ text: '🚀 Play Game!', web_app: { url: 'https://lumberjack.momis.studio' } }]]
            }
        };
