...
const LumberjackGame = ({ branches, lumberjackPos, loading, gameActive }) => {
  ...
  const renderBranches = () => {
    return branches.map((direction, index) => (
      ...
      >
        {/* شاخه‌ها */}
        {direction === 'left' && (
          <div className="absolute left-0 h-2 w-1/2 bg-[#8B4513] rounded-l-full"></div>
        )}
        
        {direction === 'right' && (
          <div className="absolute right-0 h-2 w-1/2 bg-[#8B4513] rounded-r-full"></div>
        )}
      </div>
    ));
  };
  ...
  const renderLumberjack = () => (
    <div className="relative w-full h-24 flex items-center justify-center">
      <div className="absolute w-2 h-full bg-[#5D4037]"></div>
  ...
  return (
    <div className="relative w-full max-w-md h-full flex flex-col items-center justify-center">
      ...
      <div className="w-full h-4/5 flex flex-col-reverse bg-green-700 border-4 border-green-900 rounded-lg overflow-hidden">
        ...
        {/* تنه درخت (پیوسته) */}
        <div className="absolute left-1/2 transform -translate-x-1/2 w-2 h-[calc(100%-4rem)] bg-[#5D4037]"></div>
        
        {/* شاخه‌ها (5 شاخه از پایین تا بالا) */}
